	...
	return this.each(function() {
		...
		function drawLinks() {
			...
			if (opts.display_msg) {
				if(!maxentries){
					panel
						.append('<div class="pxofy">No result is found</div>');
				}else{
				panel
						.append('<div class="pxofy">Showing results from&nbsp;'
								+ ((current_page * opts.items_per_page) + 1)
								+ '&nbsp;to&nbsp;'
								+ (((current_page + 1) * opts.items_per_page) > maxentries
										? maxentries
										: ((current_page + 1) * opts.items_per_page))
								+ '&nbsp;, there are&nbsp;' + maxentries + '&nbsp;results in total.</div>');
				}
			}

			if(opts.setPageNo){
				  panel.append("<div class='goto'><span class='text'>Jump to</span><input type='text'/><span class='page'>page</span><button type='button' class='ue-button long2'>OK</button></div>");
			}
		}
		...
		if(maxentries==0){
			panel.append('<span class="current prev">'+opts.prev_text+'</span><span class="current next">'+opts.next_text+'</span><div class="pxofy">No result is found</div>');
		}else{
			drawLinks();
		}
